...
Four pieces cover the journey from "user clicks install" to "new keyboard appears":

1. **Live download progress in the update popup** (`updateDownloadProgress(bytes, total)` signal). The install worker passes a throttled callback (emit every 256 KB, plus the final chunk so the bar lands at 100 %) into `download_and_install(progress=...)`. The QML popup that opens off the title-bar ↓ icon shows `Downloading X.X / Y.Y MB (N%)` and a determinate `ProgressBar`. When the server omits `Content-Length` the bar falls back to indeterminate and the text shows only the byte count. The throttle matters: a 64 KB chunk size on an 85 MB installer would otherwise fire ~1300 queued-signal hops. **Never expose the download URL to QML** — the bridge only ever emits primitive ints; the URL lives behind `self._update_info`.
2. **Pre-install toast in the live OSK** (`updateInstallHandoffPending` signal → `updateStartingToast` in `Main.qml`). Fired by `updater.download_and_install` via the `on_installer_launching` callback, immediately before `_launch_installer`. The bridge callback emits the signal then sleeps `_PRE_INSTALL_TOAST_DWELL_S` (1.8 s) in the worker thread so the toast paints before the installer's taskkill arrives. Without this dwell, the toast and the keyboard would both vanish before the user could read it.
3. **Relauncher splash** (`_run_with_splash` in `_update_relauncher.py`). A frameless `WindowStaysOnTopHint` widget owned by the detached relauncher process. The polling logic was refactored from blocking sleep loops into a `QTimer` state machine (`_poll_parent` → `_poll_new_exe` → `_launch`) so the splash stays painted between checks. New `_new_exe_ready` is the single-shot mirror of `_wait_for_new_exe`. Splash colours match the in-app toast. Has a ✕ dismiss button that *hides* the splash but keeps polling running (the user is dismissing the visual, not the work). The splash also carries an **indeterminate `QProgressBar`** under the phase-aware status text; NSIS silent install suppresses its own UI so there's no real percentage to show, but constant motion is the difference between "is this stuck?" and "still working". `_settle_progress(full=True)` pins the bar full on the Done phase and `_settle_progress(full=False)` empties it on a failure phase so the eye can tell the work has stopped before the dwell expires. New widgets in the splash must be added inside the layout-managed `QVBoxLayout` (height is fixed at 170 px to fit title + message + bar; bumping the bar adds without an explicit `setFixedSize` adjust will overlap).
4. **Post-update ✓ toast** (already shipped in 1.0.17, `updateAppliedToast` driven by `consumeUpdateHandoff`). Confirms the install completed.
