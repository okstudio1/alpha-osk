...
- ``build/windows/sign.py`` handles retry logic for Defender file locks.
...
SCRIPT_DIR = Path(__file__).parent.resolve()        # build/windows/
...
def freeze_lockfile() -> Path | None:
    """Capture ``pip freeze`` output as a release-time lockfile.

    Writes ``release/Alpha-OSK-Setup-{version}-requirements.lock.txt``
    containing every Python package + exact version that was resolved
    in the build venv.  PyInstaller bundles whatever pip resolved at
    build time, so this lockfile is the reproducible record of what
    actually went into the frozen exe.
    ...
    # Single source of truth -- bump src/__version__.py and rebuild.
    sys.path.insert(0, str(PROJECT_ROOT))
    from src.__version__ import __version__ as version
    lockfile = RELEASE_DIR / f"Alpha-OSK-Setup-{version}-requirements.lock.txt"
    ...
    try:
        ...
        error(f"pip freeze failed: {exc}")
        return None

    # Prepend a header so consumers (auditors, future-us) know the
    # version this lockfile pins and how it was produced.
    body = (
        f"# Alpha-OSK {version} -- Python dependency lockfile\n"
        f"# Generated by build/windows/build.py via `pip freeze --all`.\n"
    ...
    success(f"Lockfile written: {lockfile.name} ({package_count} packages)")
    return lockfile
...
def sign_build(signtool_path: str) -> bool:
    ...
    Uses ``build/windows/sign.py`` for retry logic.
    """
...
def _generate_nsi_script(version: str, installer_name: str) -> str:
    ...
    unicon_line = f'!define MUI_UNICON "{icon_path}"' if has_icon else ""

    # Branding images
    header_path = str(SCRIPT_DIR / "installer-header.bmp").replace("/", "\\")
    sidebar_path = str(SCRIPT_DIR / "installer-sidebar.bmp").replace("/", "\\")
    has_header = (SCRIPT_DIR / "installer-header.bmp").exists()
    has_sidebar = (SCRIPT_DIR / "installer-sidebar.bmp").exists()

    # EULA: clickwrap MIT license. RTF renders with bold headers and
    # plays nicely with the recent NSIS high-contrast-mode fix on the
    # license control. ``MUI_LICENSEPAGE_CHECKBOX`` forces the user to
    # tick a box before Next un-greys; that's the modern clickwrap
    # pattern most installers (and every commercial EULA guide) ship.
    # Silent install (``/S``, used by the auto-updater) bypasses pages
    # entirely so this never blocks an upgrade.
    license_path = str(SCRIPT_DIR / "LICENSE.rtf").replace("/", "\\")
    has_license = (SCRIPT_DIR / "LICENSE.rtf").exists()
    if has_license:
        license_checkbox_def = "!define MUI_LICENSEPAGE_CHECKBOX"
        license_checkbox_text = (
        ...
        license_text_top = (
            ...
            'You must accept them to install Alpha-OSK."'
        )
        license_page_insert = (
            f'!insertmacro MUI_PAGE_LICENSE "{license_path}"'
        )
    else:
        license_checkbox_def = ""
        license_checkbox_text = ""
        license_text_top = ""
        license_page_insert = (
            "; (no LICENSE.rtf found - license page skipped)"
        )

    return f"""; Alpha-OSK NSIS Installer
; Auto-generated by build/windows/build.py
...
!define APP_URL "https://github.com/okstudio1/alpha-osk"
