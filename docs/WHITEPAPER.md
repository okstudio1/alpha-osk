...
Total wall-clock budget for steps 4–5 on a 2018 laptop is comfortably under 30 ms with vocabularies up to ~100k unigrams. The dominant cost is the fuzzy beam search, which is also the component most affected by future SymSpell migration (§8 Known Gap #1).
...
2. **Edit-distance candidates** for transposition, deletion, and insertion errors, with per-edit penalties (`_TRANSPOSITION_PROB = 0.30`, `_DELETION_PROB = 0.20`, `_INSERTION_PROB = 0.15`). Apostrophe insertion is bumped to `0.50` because missing apostrophes ("im" → "I'm", "dont" → "don't") are by far the dominant insertion error in real OSK typing.

The two paths produce overlapping candidates, which are deduplicated by string and rescored against the unigram frequency table.
...
A fully unified scoring model — where the literal typed word has an explicit probability and competes against alternatives in a single ranked list — is the proper long-term fix and is described as Known Gap #2 in §8.
...
1. **SymSpell for fuzzy matching.** The current Levenshtein-based candidate generation in `fuzzy_recognizer.py` is correct but slow. SymSpell (Garbe, 2012) precomputes deletion variants at index time and reduces query to an O(1) hash lookup. Estimated 1000× speedup, ~30 MB additional RAM. Worth doing if the engine grows or if the candidate-generation budget gets tighter.
...
3. **Spatial edit costs in final ranking.** Key-distance weights from `fuzzy_recognizer` currently feed candidate *generation* but not the final rank. Folding them into the merge layer would let the engine prefer "the" over "rhe" in ambiguous contexts based on the fact that `r` is far from `t` in QWERTY.
4. **Katz / stupid backoff for sparse contexts.** Linear interpolation gives `λ₃·P_tri = 0` weight to the trigram term when the trigram has never been seen, which is correct but pessimistic — Katz backoff would discount seen events and redistribute mass to lower-order fallbacks. Larger-lift change (~100 lines) with a measurable quality gain on rare contexts.
5. **Larger seed corpus.** The ~700 / ~700 curated bigram/trigram lists are hand-audited but small. Seeding from COCA top-100k bigrams or Google n-gram exports would dwarf them. Easy win, no algorithm changes — just more data.
